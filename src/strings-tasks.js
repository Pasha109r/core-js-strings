...
function removeFirstOccurrences(str, value) {
  const occur = str.indexOf(value);
  if (occur === -1) {
    return str;
  }
  return str.slice(0, occur) + str.slice(occur + value.length);
}
...
function removeLastOccurrences(str, value) {
  const occur = str.lastIndexOf(value);
  if (occur === -1) {
    return str;
  }
  return str.slice(0, occur) + str.slice(occur + value.length);
...
function countVowels(str) {
  let count = 0;
  for (let i = 0; i < str.length; i += 1) {
    if ('aeiouyAEIOUY'.includes(str.at(i))) {
      count += 1;
    }
  }
  return count;
...
function isPalindrome(str) {
  const cS = str.toLowerCase().replace(/[^a-z0-9]/g, '');
...
function reverseWords(str) {
  ...
  for (let i = 0; i < revStr.length; i += 1) {
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, function a(char) {
    ...
    const set = (char.charCodeAt(0) - base + 13) % 26;
    return String.fromCharCode(base + set);
  });
}
